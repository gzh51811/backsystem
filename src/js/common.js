...
/*
 movePullMenu(box, menu)
 	参数：
 		box ：外层节点
 		memu ：隐藏的菜单节点
 **/
function movePullMenu(box, menu) {
	//事件
...
/*
 oncliickPullMenu(head,menu);
 点击出现下来菜单
	 head:兄弟节点
	 menu:隐藏的节点
 */
function onclickPullMenu(head, menu) {
	var show = true;
	head.onclick = function(e) {
		menu.style.display = show > 0 ? "block" : "none";
		show = !show;
	...
	bind(document, 'click', function(){
		menu.style.display ="none";show=true;
	});
	
}
//点击缓慢出现下拉菜单

function onclickPullMenu1(head, menu) {
	var show = true;
	head.onclick = function(e) {
		var height = show > 0 ? "200" : "0";
		startMove(menu,{'height':height});
		show = !show;
		if(e.stopPropagation){
			e.stopPropagation();
		}else{
...
/*
 	onclickTab(aLis,aDivs);
 * 		aLis：多个li的节点
 * 		aDivs：多个div的节点,显示一个div
 * 
 * 
 * */
function onclickTab(aLis, aDivs) {
...
/*
 	moveTab(aLis,aDivs)
 *		aLis:多个li节点
 *	 	aDivs：多个div节点，显示一个div
 
 * */
function moveTab(aLis, aDivs) {
	for(var i = 0; i < aLis.length; i++) {
		aLis[i].index = i;
...
/*
 	对象转字符串：objToStr(obj);
 	参数:
 		对象obj   {key0：0,key1：1,key2：2}
 * 		返回值 str字符串， key0=0&key1=1&key2=2
 * */
...
function previousSibling(brother) {
	if(brother.previousElementSibling){
		//高级浏览器
		return brother.previousElementSibling;
	}else{
		return brother.previousSibling;
	}
}
/*
 	查找首节点：
...
function nextSibling(brother) {
	if(brother.nextElementSibling){
		//高级浏览器
		return brother.nextElementSibling;
	}else{
...
var checkReg={
	...
	accountNum:function(str){//用户名
		var reg=/^\w[\w]{5,19}$/
		return reg.test(str);
	},
	nikename:function(str){//昵称:至少一个中文
		var reg=/^[\u2E80-\u9FFF]+$/
		return reg.test(str);
	},
	...
	urladr:function(str){//网址
		var reg=/[a-zA-Z0-9][-a-zA-Z0-9]{0,62}(\.[a-zA-Z0-9][-a-zA-Z0-9]{0,62})+\.?/;
		return reg.test(str);
	},
	floatnumber:function (str) {
		var reg = /^[0-9]+(.[0-9]{1,2})?$/;
		return reg.test(str);
	},
	intnumber:function (str) {
		var reg = /^[1-9]\d*$/;
		return reg.test(str);
	}
}
 ...
 * 				<div id="banner">
					...
					<div class="btn">
						<div class="preBtn">&lt</div>
						<div class="nextBtn">&gt</div>
					</div>
				</div>
 * */
function lunbotu(banner,active){
		var banner=getid("banner");
		var tupian=banner.getElementsByClassName("tupian")[0];
		var alis=tupian.getElementsByTagName("li");
		var jiaodian=banner.getElementsByClassName("jiaodian")[0];
		var aspans=jiaodian.getElementsByTagName("span");
		...
		var next = function(){
			...
			startMove(alis[num], {'left' : 0});
			spanNow();//调用
		}
		
		//焦点跟随
		var spanNow = function(){
		...
		timer = setInterval(next, 2500);//每隔两秒切换一个图片
		
		//3.点击上下按钮：可以切换下一张和上一张
		
		//鼠标进入可视区，停止定时器，移开又开始自动轮播
		css(preBtn,"opacity",0);
		css(nextBtn,"opacity",0);
		banner.onmouseover = function(){
			clearInterval(timer);
			css(preBtn,"opacity",1);
			css(nextBtn,"opacity",1);
		}
		...
		for(var i = 0; i < aspans.length; i++) {
			...
			aspans[i].onclick = function() {
					...
					startMove(alis[this.index], {'left' : 0});
...
$(function(){
    let user = localStorage.getItem('user');
	 if(!user){
       ...
       let ele= $('.layui-side .layui-nav .layui-nav-item').eq(1);
        if(!user.admin){
						ele.find('dd:eq(0)').css('display','block');
						ele.find('dd:eq(1)').css('display','none');
						ele.find('dd:eq(2)').css('display','none');
					}else{
						ele.find('dd').css('display','block');		
						ele.find('dd:eq(0)').css('display','none');
						
					}
		if(user.admin=='big'){
						
			$('#name').html('超级管理员--'+user.nickname);
		}else if(user.admin=='small'){

			$('#name').html('普通管理员--'+user.nickname);				
		}else{
